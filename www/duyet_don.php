...
<body id="duyetdon">
   ...
   <table cellpadding="10" cellspacing="10" border="0" class="table-bordered table-hover m-auto bg-light">
        ...
        if($type=="Giám đốc"){
            ...
            if ($stm->rowCount()>0){
                while ($nghiphep = $stm->fetch(PDO::FETCH_ASSOC)){
                    ...
                    echo' <td>'.$total.'</td>';
                    echo' <td>'.$reason.'</td>';
                    echo '<td><button type="button" class="btn btn-light"> <a href="duyetdon_chitiet.php?idnv='.$id_nv.'&id_form='.$id_form.'&id='.$id.'">Chi tiết</a></button></td>';
                    echo' </tr>';
                }
    ...
    <div>
        ...
        <button type="button" class="btn btn-light mt-1 ml-3 float-right  "> <a href="tatcadon.php?id=<?=$_SESSION['id']?>">Tất cả đơn</a></button>
    </div>
    </div>
</body>
</html>